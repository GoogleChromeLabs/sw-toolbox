...
var validatePrecacheInput = function(items) {
  items.forEach(function(item) {
    if (typeof item !== 'string' && !(item instanceof Request)) {
      throw new TypeError('The input to precache must result in an array ' +
        'of strings and/or Requests only.');
    }
  });
...
function precache(items) {
  if (Array.isArray(items)) {
    ...
    throw new TypeError('precache expects an array of strings or a ' +
