...
function fetchAndCache(request, options) {
  debug('fetchAndCache:' + request.url);
  options = options || {};
    ...
    if (request.method === 'GET' && successResponses.test(response.status)) {
      openCache(options).then(function(cache) {
        ...
        if (cacheOptions.notifyOnCacheUpdate) {
          debug('notifyOnCacheUpdate is set: ' + request.url);
          readyForCacheUpdate = cache.match(request)
            .then(function(cachedResponse) {
              if (cachedResponse && !areResponsesTheSame(cachedResponse, response)) {
                debug('areResponsesTheSame was false.');
                notifyClientsOfUpdate(request.url);
              } else {
                debug('areResponsesTheSame was true.');
              }
            });
        }
...
function areResponsesTheSame(responseA, responseB) {
  console.log(responseB.headers);
  return ['content-length', 'last-modified', 'etag', 'date'].every(function(header) {
    console.log(header, responseA.headers.get(header));
    console.log(header, responseB.headers.get(header));
