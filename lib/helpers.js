  ...
  return fetch(request.clone()).then(function(response) {
    ...
    if (request.method === 'GET' && successResponses.test(response.status)) {
      openCache(options).then(function(cache) {
        cache.put(request, response).catch(function(error) {
          console.warn('Unable to cache', request.url, 'due to', error);
        }).then(function() {
...
function cleanupCache(request, cache, cacheOptions) {
  ...
  return idbCacheExpiration.getDb(cacheName).then(function(db) {
    ...
    // Just log this error, and don't treat it as being fatal.
    // Make sure that we still run our cache expiration logic even if setting
    // the timestamp in IDB fails to due a quota error. We want to allow
    // expiration to run, since it might free up space.
    debug(error);
  }).then(function(db) {
