...
function fetchAndCache(request, options) {
  ...
  return fetch(request.clone()).then(function(response) {
    console.log('fetchAndCache() Got Response');
    ...
    if (request.method === 'GET' && successResponses.test(response.status)) {
      console.log('fetchAndCache() openCache()');
      openCache(options).then(function(cache) {
        console.log('fetchAndCache() openCache() go cache - putting now');
        cache.put(request, response).then(function() {
          console.log('fetchAndCache() openCache() put done.');
          ...
          if ((cacheOptions.maxEntries || cacheOptions.maxAgeSeconds) &&
              cacheOptions.name) {
            console.log('Queueing up expiration');
...
function queueCacheExpiration(request, cache, cacheOptions) {
  console.log('queueCacheExpiration');
...
function cleanupCache(request, cache, cacheOptions) {
  ...
  return idbCacheExpiration.setTimestampForUrl(cacheName, requestUrl, now)
  .then(function() {
    return idbCacheExpiration.expireEntries(cacheName,
      maxEntries, maxAgeSeconds, now);
  })
