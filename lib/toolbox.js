...
self.addEventListener('install', function(event) {
  ...
  event.waitUntil(
    ...
    .then(function(cache) {
      return Promise.all(this.options.preCacheItems)
      .then(flatten)
...
self.addEventListener('activate', function(event) {
  ...
  var inactiveCache = this.options.cache.name + '$$$inactive$$$';
  event.waitUntil(helpers.renameCache(inactiveCache, this.options.cache.name));
});

// Fetch

// self.addEventListener('fetch', function(event) {
//   var handler = this.router.match(event.request);

//   if (handler) {
//     event.respondWith(handler(event.request));
//   } else if (this.router.default && event.request.method === 'GET') {
//     event.respondWith(this.router.default(event.request));
//   }
// });
