...
function closeDatabase(db, promiseChain) {
  var closeDb = function(result) {
    db.close();
    return result;
  };

  return promiseChain.then(closeDb, closeDb);
}

function setTimestampForUrl(cacheName, url, now) {
  console.log('setTimestampForUrl', cacheName, url, now);
  return openDb(cacheName)
  .then(function(db) {
    ...
    return closeDatabase(db, promiseChain);
  });
}
...
function expireEntries(cacheName, maxEntries, maxAgeSeconds, now) {
  console.log('expireEntries', cacheName, maxEntries, maxAgeSeconds, now);
  return openDb(cacheName)
  .then(function(db) {
    var promiseChain = expireOldEntries(db, maxAgeSeconds, now)
    .then(function(oldUrls) {
      return expireExtraEntries(db, maxEntries)
      .then(function(extraUrls) {
